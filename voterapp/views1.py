...
def get_voters_by_booth(request, booth_id):
    ...
    for row in results:
        voters.append({
            ...
            'voter_in_city_id' : row[13],
            'voter_current_location' : row[14],
            'voter_name_mar' : row[15]
...
class PoliticianLoginView(APIView):
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            try:
                # Check if input is a contact number or username
                if username_or_contact.isdigit():
                    politician = Politician.objects.get(politician_contact_number=username_or_contact)
                else:
                    politician = Politician.objects.get(politician_name=username_or_contact)
...
class Favour_non_favourRetriveUpdateDestroy(APIView):
    ...
    def handle_bulk_update(self, request):
        ...
        try:
            # Bulk update the `voter_favour_id` for the provided voter_ids
...
#             # Create an instance of Town_user with the validated data
#             town_user = Town_user(**user_data)  
#             town_user.save()  # This should work correctly now
...
def get_voters_by_userwise(request, user_booth_user_id):
    with connection.cursor() as cursor:
        cursor.execute("""
            ...
            LEFT JOIN
                 tbl_marital_status m ON v.voter_marital_status_id = m.marital_status_id
            LEFT JOIN 
...
def total_voters(request):
    try:
        sql_query = "SELECT voter_id, voter_name, voter_favour_id, voter_name_mar  FROM tbl_voter ; "
        
        with connection.cursor() as cursor:
            cursor.execute(sql_query)
            rows = cursor.fetchall()
        
        voter_list = [{'voter_id': row[0], 'voter_name': row[1], 'voter_favour_id': row[2], 'voter_name_mar': row[3]} for row in rows]
...
def get_booth_user_info(request):
    with connection.cursor() as cursor:
        ...
        for row in data:
            ...
            #typecasting booth_ids to integer
...
#             row_dict['booth_names'] = row_dict['booth_names'].split(', ')
...
#         # Fetch column names and replace 'favour_id' with 'voter_favour_id' in the output
#         columns = [desc[0] for desc in cursor.description]
#         columns = ["voter_favour_id" if col == "favour_id" else col for col in columns]
        
#         # Build the response data with the modified column names
...
#                             GROUP_CONCAT(DISTINCT u.user_name SEPARATOR ', ') AS booth_user_names,
#                             GROUP_CONCAT(DISTINCT u.user_phone SEPARATOR ', ') AS booth_user_phones,
#                             GROUP_CONCAT(DISTINCT u.user_id SEPARATOR ', ') AS booth_user_ids,
#                             t.town_id,  
#                             t.town_name,
#                             ps.panchayat_samiti_name  -- Add taluka_name to the SELECT statement
#                         FROM 
#                             vw_voter_list v
#                         LEFT JOIN 
...
#                         LEFT JOIN 
#                             tbl_panchayat_samiti ps ON t.town_panchayat_samiti_id = ps.panchayat_samiti_id  -- Join with taluka table
#                         GROUP BY 
#                             b.booth_id, b.booth_name, t.town_id, t.town_name, ps.panchayat_samiti_name  -- Add taluka_name to the GROUP BY clause
#                         ORDER BY 
#                             b.booth_id;
...
def booth_votes_summary(request):
        ...
        cursor.execute("""
            ...
            FROM (
                ...
                SELECT 
                    ...
                    COUNT(DISTINCT v.voter_id) AS voter_count,  -- Ensure voter count is based on distinct voters
                ...
                GROUP BY 
                    ...
                    b.booth_id, b.booth_name, t.town_id, t.town_name, ps.panchayat_samiti_name
    ...
    for row in rows:
        ...
        result.append({
            ...
            'user_name': ', '.join(user_names),  # Comma-separated usernames

            'user_phone': ', '.join(user_phones),  # Comma-separated user phones

            'user_id': user_ids,  # List of user IDs

            'mycount': row[3],  # Voter count remains unchanged
...
#     # Use the fetched constituency name
...
#     query = """
#     SELECT voter_id, voter_name, voter_contact_number, voter_vote_confirmation_id, favour_id 
#     FROM vw_voter_list 
#     WHERE religion_id = %s
#     """
...
def get_voters_by_religion(request, religion_id):
    ...
    query = """
    SELECT 
    v.voter_id,          -- Add voter_id
    v.voter_name,
    v.voter_contact_number,  -- Add voter_contact_number
    c.cast_name,
    r.religion_name
FROM 
    vw_voter_list v
...
def get_voters_by_booth_user_and_cast(request, user_booth_id, cast_id):
    try:
        ...
        columns = ['voter_id', 'voter_name', 'voter_contact_number', 'cast_name', 'voter_name_mar']
...
def age_wise_voter(request, age_from, age_to):
    ...
    query = """
    SELECT voter_id, voter_name, voter_name_mar, voter_age 
    FROM tbl_voter 
...
class VoterByBoothUserConfirmationView(APIView):
    def get(self, request, user_id, confirmation_id):
        if confirmation_id == 2:
            ...
            sql = """
                select v.voter_id, v.voter_name, v.voter_vote_confirmation_id FROM tbl_voter v where voter_booth_id IN 
...
class VoterByTownUserConfirmationView(APIView):
    def get(self, request, town_user_id, confirmation_id):
        if confirmation_id == 2:
            confirmation_id = None
            sql = """
                SELECT v.voter_id, v.voter_name, v.voter_vote_confirmation_id, v.voter_name_mar 
                FROM tbl_voter v 
                WHERE voter_town_id IN 
                (SELECT user_town_town_id FROM tbl_user_town WHERE user_town_town_user_id = %s) 
                AND v.voter_vote_confirmation_id IS NULL;
            """
            params = [town_user_id]
        else:
            sql = """
                ...
                WHERE voter_town_id IN 
                (SELECT user_town_town_id FROM tbl_user_town WHERE user_town_town_user_id = %s) 
                AND v.voter_vote_confirmation_id = %s;
            """
...
def get_voters_by_group_id(request, voter_group_id):
    if request.method == 'GET':
        with connection.cursor() as cursor:
            cursor.execute("""
                SELECT voter_id, voter_name, voter_contact_number, voter_name_mar
                FROM tbl_voter
...
# def booth_voting_view(request):
#     with connection.cursor() as cursor:
#         cursor.callproc('get_booth_voting_percentage')
...
def booth_voting_view(request, booth_id=None):
    """
    Fetch voting data for all booths or a specific booth based on booth_id.
    """
    with connection.cursor() as cursor:
...
# def town_voting_view(request):
#     with connection.cursor() as cursor:
#         cursor.callproc('get_town_voting_percentage')
...
def town_voting_view(request, town_id=None):
    with connection.cursor() as cursor:
        # Pass town_id as a parameter to the stored procedure
...
def get_temp_voter_data_town(request, temp_voter_data_updated_by_user_id):
    with connection.cursor() as cursor:
        # Fetch temp voter data along with the voter name if temp voter name is not present
        cursor.execute("""
            SELECT
...
#         query = """
#            SELECT 
#                 ROW_NUMBER() OVER (ORDER BY town_name) AS serial_number,
#                 town_name, 
...
#         pdf.drawString(x_offset, y_start, "Voter Count by Town")
#         pdf.setFont("Helvetica", 12)

#         headers = ["S/N", "Town Name", "Voter Count"]
#         header_widths = [50, 200, 200]

#         y = y_start - line_height
#         for i, header in enumerate(headers):
#             pdf.drawString(x_offset + sum(header_widths[:i]), y, header)

#         pdf.line(x_offset, y - 5, width - 50, y - 5)
...
#                 pdf.showPage()  
#                 pdf.setFont("Helvetica-Bold", 16)
#                 pdf.drawString(x_offset, height - 50, "Voter Count by Town")  
#                 pdf.setFont("Helvetica", 12)
#                 for i, header in enumerate(headers):
#                     pdf.drawString(x_offset + sum(header_widths[:i]), height - 70, header)
#                 pdf.line(x_offset, height - 75, width - 50, height - 75)
#                 y = height - 90  

#             pdf.drawString(x_offset, y, str(row[0]))  
#             pdf.drawString(x_offset + 50, y, row[1])  
#             pdf.drawString(x_offset + 250, y, str(row[2]))  
#             y -= line_height

#         pdf.line(x_offset, y_start, x_offset + table_width, y_start)  
#         pdf.line(x_offset, y_start - (line_height * (len(rows) + 1)), x_offset + table_width, y_start - (line_height * (len(rows) + 1)))  # Bottom border
...
#         response['Content-Disposition'] = 'attachment; filename="voter_count.pdf"'
#         return response
...
#         # Headers
#         headers = ["S/N", "Town Name", "Voter Count", "Town Users", "Sarpanch Name"]
#         header_widths = [50, 150, 100, 200, 150]
...
#             pdf.drawString(x_offset, y, str(row[0]))  # Serial number
#             pdf.drawString(x_offset + 50, y, row[1])  # Town name
#             pdf.drawString(x_offset + 200, y, str(row[2]))  # Voter count
#             pdf.drawString(x_offset + 300, y, row[3] or '')  # Town user names
#             pdf.drawString(x_offset + 500, y, row[4] or '')  # Sarpanch name

#             y -= line_height

#         pdf.line(x_offset, y_start, x_offset + table_width, y_start)  # Top border
#         pdf.line(x_offset, y_start - (line_height * (len(rows) + 1)), x_offset + table_width, y_start - (line_height * (len(rows) + 1)))  # Bottom border
...
class VoterCountPDFView(APIView):
    def get(self, request):
        ...
        for row in rows:
            town_users = wrap_text(row[3] or '', max_length=40)  
            sarpanch_name = wrap_text(row[4] or '', max_length=20)  
            table_data.append([str(row[0]), row[1], str(row[2]), town_users, sarpanch_name])

        col_widths = [0.5 * inch, 2.5 * inch, 1.0 * inch, 1.8 * inch, 1.8 * inch] 
        ...
        table.setStyle(TableStyle([
            ...
            ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'),
            ('GRID', (0, 0), (-1, -1), 1, colors.black),  # Border grid for all cells
...
def get_non_voted_voters(request):
    with connection.cursor() as cursor:
        cursor.execute("SELECT voter_id, voter_name,voter_name_mar  FROM vw_voter_list WHERE voter_vote_confirmation_id IS NULL;")
        rows = cursor.fetchall()
...
class VotedVotersListByTownUser(View):
    def get(self, request, user_town_user_id, voter_vote_confirmation_id):
        ...
        with connection.cursor() as cursor:
            cursor.execute(query_voters, [user_town_user_id])
            voters = cursor.fetchall()
            cursor.execute(query_count, [user_town_user_id])
            count = cursor.fetchone()[0]

        voters_list = [{'voter_id': row[0], 'voter_name': row[1], 'voter_vote_confirmation_id': row[2]} for row in voters]
...
#         if voter_vote_confirmation_id == 2:
#             query_voters = """
#                 SELECT voter_id, voter_name, voter_vote_confirmation_id from tbl_voter where voter_booth_id IN (select user_booth_booth_id
#             FROM tbl_user_booth where user_booth_user_id = %s) AND voter_vote_confirmation_id IS NULL;

#             """

#             query_count = """
#                 SELECT count(*) from tbl_voter where voter_booth_id IN (select user_booth_booth_id
#                 FROM tbl_user_booth where user_booth_user_id = %s) AND voter_vote_confirmation_id IS NULL;  
...
#             query_voters = """
#                 SELECT voter_id, voter_name, voter_vote_confirmation_id from tbl_voter where voter_booth_id IN (select user_booth_booth_id
#             FROM tbl_user_booth where user_booth_user_id = %s) AND voter_vote_confirmation_id = 1;
#             """
    ...
    if request.method == 'GET':
        try:
            with connection.cursor() as cursor:
                cursor.execute(
                    """
                    SELECT voter_group_id FROM tbl_voter_group
                    WHERE voter_group_voter_group_user_id = %s
                    """, [voter_group_user_id]
                )
                voter_group_ids = [row[0] for row in cursor.fetchall()]

                if not voter_group_ids:
                    return JsonResponse({'message': 'No voter groups found for this user.'}, status=404)
                
                cursor.execute(
                    """
                    SELECT voter_id, voter_name, voter_name_mar, voter_favour_id, voter_contact_number,voter_vote_confirmation_id,booth_name, booth_name_mar
                    FROM tbl_voter INNER JOIN tbl_booth on voter_booth_id = booth_id
                    WHERE voter_voter_group_id IN %s
                    """, [tuple(voter_group_ids)]
                )
 
                voters = cursor.fetchall()

            voter_list = [{'voter_id': voter[0], 'voter_name': voter[1], 'voter_name_mar': voter[2],'voter_favour_id': voter[3],'voter_contact_number': voter[4],'voter_vote_confirmation_id': voter[5],'booth_name': voter[6]} for voter in voters]
...
#     """
#     API to fetch voter group details with voter count and associated user information.
#     """
#     query = """
#         SELECT 
#             v.voter_voter_group_id,
#             vgu.voter_group_user_id,
#             vgu.voter_group_user_name,
#             vgu.voter_group_user_contact_number,
#             vg.voter_group_name,
#             COUNT(CASE WHEN v.voter_id IS NOT NULL THEN 1 END) AS group_voter_count
#         FROM 
#             tbl_voter v
#         JOIN 
#             tbl_voter_group vg ON v.voter_voter_group_id = vg.voter_group_id
#         JOIN 
#             tbl_voter_group_user vgu ON vg.voter_group_voter_group_user_id = vgu.voter_group_user_id
#         GROUP BY 
#             v.voter_voter_group_id,
#             vgu.voter_group_user_id,
#             vgu.voter_group_user_name,
...
def get_voter_group_details(request):
    if request.method == 'GET':
        try:
            with connection.cursor() as cursor:
                # Step 1: Get voter group details for all users
                cursor.execute("""
    SELECT vg.voter_group_name,
           COUNT(v.voter_id) AS group_voter_count,
           vgu.voter_group_user_id,
    ...
    GROUP BY vg.voter_group_name, vgu.voter_group_user_id, vgu.voter_group_user_name, vgu.voter_group_user_contact_number
    ORDER BY vg.voter_group_name, vgu.voter_group_user_name
""")
            ...
            if group_details:
                ...
                group_users = [
                    {
                        'voter_group_user_id': row[2],
                        'voter_group_name': row[0],
                        'group_voter_count': row[1],
                        'group_user_name': row[3],
                        'group_user_contact': row[4]
                ...
                return JsonResponse({'error': 'No group users found.'}, status=404)
    ...
    if request.method == 'GET':
        try:
            ...
            with connection.cursor() as cursor:
                ...
                cursor.execute(f"""
                    SELECT v.voter_id, v.voter_name, v.voter_name_mar, v.voter_vote_confirmation_id, v.voter_favour_id
                    FROM tbl_voter v
                    WHERE v.voter_voter_group_id = %s AND ({confirmation_condition})
                """, [voter_group_id])
...
def get_voter_count_pdf(request, town_area_type_id=None):
    try:
        ...
        if voter_data_response.status_code != 200:
            return voter_data_response  # Return the error response if applicable
        ...
        column_positions = [30, 210, 310, 380, 450, 520]  # X positions for columns based on new widths
        column_widths = [180, 100, 70, 70, 70, 70]  # Updated column widths
        y_position = 700  # Initial Y position for table

        # Draw table header with boundaries
        pdf.setFont("Helvetica-Bold", 10)  # Ensure bold font for headers
        for i, header in enumerate(headers):
            wrapped_header = truncate_text(header, column_widths[i] - 10, pdf)
            pdf.drawString(column_positions[i] + 5, y_position - 10, wrapped_header)  # Add some padding
            pdf.rect(column_positions[i], y_position - 20, column_widths[i], 20)  # Draw header box

        # Draw table rows
        pdf.setFont("Helvetica", 10)  # Regular font for row data
        y_position -= 40  # Move down for the first row
        ...
        for booth in voter_data:
            if y_position < 50:  
                pdf.showPage()
                y_position = 750
                pdf.setFont("Helvetica-Bold", 10)  # Redraw headers in bold
                for i, header in enumerate(headers):
                    wrapped_header = truncate_text(header, column_widths[i] - 10, pdf)
            ...
            # Split user names and phones into lists
            ...
            # Handle multiple names and phones, and print each on a new line
            max_lines = max(len(booth_user_names_list), len(booth_user_phones_list))
            
            for i in range(max_lines):
                # Get the user name and phone number for this line
                ...
                for i, data in enumerate(cell_data):
                    ...
                    pdf.rect(column_positions[i], y_position, column_widths[i], row_height)  # Draw cell box
    ...
    if request.method == 'GET':
        try:
            with connection.cursor() as cursor:
                cursor.execute(
                    """
                    SELECT voter_id, voter_name, voter_name_mar,voter_favour_id
                    FROM tbl_voter
            ...
            voter_list = [{'voter_id': row[0], 'voter_name': row[1], 'voter_name_mar': row[2], 'voter_favour_id': row[3]} for row in voters]
...
def get_voters_by_favour_id(request, favour_id):
    if request.method == 'GET':
        try:
            ...
            # Format the voter data
            voter_list = [{'voter_id': row[0], 'voter_name': row[1], 'booth_name': row[2]} for row in voters]
            total_voters = len(voter_list)

            # Check if the response should be in PDF format
            if request.GET.get('format') == 'pdf':
                pass
                # return generate_pdf_voter_list(voter_list, total_voters)
