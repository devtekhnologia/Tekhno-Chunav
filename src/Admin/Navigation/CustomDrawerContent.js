...
import { dropTotalVotersTable } from '../../Db/TotalVotersHelper';
import { dropTotalVotersBGTable, dropVotedVoterTable } from '../Voters/SQLiteHelper';
import { closeDatabase, dropTownsTable } from '../../Db/TownTableHelper';
import { dropBoothsTable } from '../../Db/BoothTableHelper';
...
const CustomDrawerContent = ({ navigation }) => {
    ...
    const unInialize = async () => {
        await dropTotalVotersTable();
        await dropTotalVotersBGTable();
        await dropVotedVoterTable();
        await dropTownsTable();
        await dropBoothsTable();
        await closeDatabase();
            ...
            >
                ...
                <View style={{ marginTop: 20 }}>
                    ...
                    >
                        ...
                        <Text style={styles.languageToggleText}>
                            {language === 'en' ? 'Marathi' : 'इंग्रजी'}
                        </Text>
                    </TouchableOpacity>

                </View>

                {/* <Pressable style={styles.logOutView} onPress={unInialize}>
                    <Feather name="refresh-cw" size={25} color="#3C4CAC" />
                    <Text style={{ color: '#3C4CAC', fontSize: 20 }}>{language === 'en' ? 'Reset Data' : 'डेटा रिसेट करा'}</Text>
                </Pressable> */}
            </LinearGradient >
        </View >
    );
