...
# # with accurate names and its count as well
...
#                 regex_name = r"(?<!and\s)(?<!Husbands\s)(?<!Fathers\s)(?<!Mothers\s)Name: :(.*?)(?=\n|$)"
#                 names = [name.strip() for name in re.findall(regex_name, result_text)]

#                 # Adjusted regex to handle spaces within voter IDs
#                 regex_serial_voter_id = r"(?:(\d+)\s+)?([A-Z]+\s?\d{1,12})"
#                 serial_voter_ids = re.findall(regex_serial_voter_id, result_text)

#                 print('serialvoter', serial_voter_ids)

#                 # Filter only valid voter IDs with both alphabets and numbers and length greater than 9
#                 valid_serial_voter_ids = [
#                     (serial.strip() if serial else '', voter_id.replace(" ", ""))
#                     for serial, voter_id in serial_voter_ids
#                     if re.search(r'[A-Za-z]', voter_id) and re.search(r'\d', voter_id) and len(voter_id.replace(" ", "")) > 9
#                 ]

#                 print('validserialvoter', valid_serial_voter_ids)
    ...
    if request.method == 'POST' and request.FILES.getlist('pdf_files'):
        ...
        try:
            ...
            for pdf_file in pdf_files:
                ...
                # # regex_serial_voter_id = r"(\d+)\s+(?:\d+\s+)?([A-Z]{2,10}\d{5,10})"
                # regex_serial_voter_id = r"(\d+)\s+(?:\d+\s+)?([A-Z0-9]{8,12})"
                # serial_voter_ids = re.findall(regex_serial_voter_id, result_text)
                ...
                # # Regex to extract serial numbers and voter IDs
                # regex_serial_voter_id = r"(?:(\d+)\s+)?([A-Z0-9]{8,12})"
                # serial_voter_ids = re.findall(regex_serial_voter_id, result_text)

                # print('serialvoter', serial_voter_ids)

                # # Filter only valid voter IDs with both alphabets and numbers
                # valid_serial_voter_ids = [
                #     (serial if serial else '', voter_id) 
                #     for serial, voter_id in serial_voter_ids 
                #     if re.search(r'[A-Za-z]', voter_id) and re.search(r'\d', voter_id)
                # ]

                # print('validserialvoter', valid_serial_voter_ids)

                # # Skipping the first 3 entries
                # valid_serial_voter_ids = valid_serial_voter_ids[2:]
                ...
                # # Skipping the first 6 entries
                # valid_serial_voter_ids = valid_serial_voter_ids[6:]
...
# gdfgdsgsdfgs
                # Regex for names (ignoring the "and" in "Name: :")
                # regex_name = r"(?<!and )Name: :(.*?)(?=\n|$)"
                # regex_name = r"(?<!and\s)(?<!Husbands\s)Name: :(.*?)(?=\n|$)"
                regex_name = r"(?<!and\s)(?<!Husbands\s)(?<!Fathers\s)(?<!Mothers\s)Name: :(.*?)(?=\n|$)"
                names = [name.strip() for name in re.findall(regex_name, result_text)]
                ...
                valid_serial_voter_ids = [
                    (serial.strip() if serial else '', voter_id.replace(" ", ""))
                    for serial, voter_id in serial_voter_ids
                    if re.search(r'[A-Za-z]', voter_id) and re.search(r'\d', voter_id) and len(voter_id.replace(" ", "")) > 8
                ]

                print('validserialvoter', valid_serial_voter_ids)

                valid_serial_voter_ids = valid_serial_voter_ids[1:]
